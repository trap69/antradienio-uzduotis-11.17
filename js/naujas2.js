        ...
        if (xh.readyState === 4 && xh.status === 200) {
            ...
            for (let i = 0; i < datos.forecastTimestamps.length - 1; i++) {
                if (datos.forecastTimestamps[i].forecastTimeUtc.slice(0, 10)
                    ...
                    let option = document.createElement("option");
                    let miestailist = document.getElementById("datosList");
...
/*let prognoze1 = document.createElement('h1');
let prognozestext = document.createTextNode(prognoze1);
let tekstas = document.createTextNode(miestas.value + " " + date + " " + "Prognoze:")
prognoze1.appendChild(createLentele);
 */
function lentele() {
            ...
            conditions.appendChild(tables);
                ...
                for (let i = 0; i < weather.forecastTimestamps.length; i++) {
                    if ( date === weather.forecastTimestamps[i].forecastTimeUtc.slice(0, 10)) {
                        ...
                        let textH = document.createTextNode(weather.forecastTimestamps[i].forecastTimeUtc.slice(10, 13) + "H" + " " + weather.forecastTimestamps[i].airTemperature + '\u00B0C');
                        let td2 = document.createElement('td');
                        let textC = document.createTextNode("skaiciai" + weather.forecastTimestamps[i].airTemperature + '\u00B0C');
                        ...
                        switch (weather.forecastTimestamps[i].conditionCode){
                            case  "clear": icons.classList.add("fas","fa-sun"); break;
                            case  "isolated-clouds": icons.classList.add("fas","fa-cloud"); break;
                            case  "scattered-clouds": icons.classList.add("fas","fa-cloud-sun"); break;
                            case  "overcast": icons.classList.add("fas","fa-cloud-sun"); break;
                            case  "light-rain": icons.classList.add("fas","fa-cloud-rain"); break;
                            case  "moderate-rain": icons.classList.add("fas","fa-cloud-showers-heavy"); break;
                            case  "heavy-rain": icons.classList.add("fas","fa-cloud-showers-heavy"); break;
                            case  "sleet": icons.classList.add("fas","fa-cloud-meatball"); break;
                            case  "light-snow": icons.classList.add("fas","fa-snowflake"); break;
                            case  "moderate-snow": icons.classList.add("fas","fa-snowflake"); break;
                            case  "heavy-snow": icons.classList.add("fas","fa-snowflake"); break;
                            case  "fog": icons.classList.add("fas","fa-smog"); break;
                            case  "na": icons.classList.add("fas","fa-question"); break;
                        }
                    }
                }
